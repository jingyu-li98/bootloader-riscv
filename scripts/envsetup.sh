...
function build_rv_perf_tool()
{
	...
	echo -e "make clean" >> build-perf.sh
	echo -e "make" >> build-perf.sh
	echo -e "sudo cp perf /usr/bin/" >> build-perf.sh
	echo -e "popd\n" >> build-perf.sh
	popd

	if [ ! -d $RV_TOOLS_DST_DIR ]; then
		mkdir -p $RV_TOOLS_DST_DIR
	fi

	sudo mv $RV_KERNEL_SRC_DIR/build-perf.sh $RV_TOOLS_DST_DIR
	sudo mv $RV_KERNEL_SRC_DIR/perf-*.tar $RV_TOOLS_DST_DIR

	pushd $RV_TOOLS_DST_DIR
	tar -xvf perf-*.tar
	rm -rf perf-*.tar
	popd
}

function clean_rv_perf_tool()
{
	rm -rf $RV_TOOLS_DST_DIR/*
}
